    ...
    T: key::secp256k1::ReadOnly + key::secp256k1::SignOnly + Clone,
{
    ...
    pub async fn issue_with_ethers(&self) -> io::Result<H256> {
        ...
        let mut tx_req = ethers::prelude::Eip1559TransactionRequest::new()
        ...
        if let Some(max_priority_fee_per_gas) = &self.max_priority_fee_per_gas {
            tx_req = tx_req.max_priority_fee_per_gas(ethers::prelude::U256::from(
                max_priority_fee_per_gas.as_u128(),
            ));
        }
        if let Some(max_fee_per_gas) = &self.max_fee_per_gas {
            tx_req = tx_req.max_fee_per_gas(ethers::prelude::U256::from(max_fee_per_gas.as_u128()));
        }
        if let Some(gas_limit) = &self.gas_limit {
