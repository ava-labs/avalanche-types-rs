...
use crate::rpcchainvm::database::{rpcdb::error_to_error_code, DatabaseError};
use avalanche_proto::{
...
pub struct Server {
    pub inner: Arc<RwLock<Box<dyn crate::rpcchainvm::database::Database + Send + Sync>>>,
}

/// A gRPC server which wraps a rpcchainvm::database::Database impl allowing client control over over RPC.
impl Server {
    pub fn new(
        db: Box<dyn crate::rpcchainvm::database::Database + Send + Sync>,
    ) -> impl avalanche_proto::rpcdb::database_server::Database {
        Server {
...
#[tonic::async_trait]
impl avalanche_proto::rpcdb::database_server::Database for Server {
