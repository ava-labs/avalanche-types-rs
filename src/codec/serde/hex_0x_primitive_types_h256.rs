...
pub fn serialize<S>(x: &primitive_types::H256, serializer: S) -> Result<S::Ok, S::Error>
where
    S: Serializer,
{
...
pub fn deserialize<'de, D>(deserializer: D) -> Result<primitive_types::H256, D::Error>
where
    D: Deserializer<'de>,
{
    ...
    primitive_types::H256::from_str(s).map_err(serde::de::Error::custom)
}

pub struct Hex0xH256(primitive_types::H256);

impl SerializeAs<primitive_types::H256> for Hex0xH256 {
    fn serialize_as<S>(x: &primitive_types::H256, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        serializer.serialize_str(&format!("0x{:x}", *x))
...
impl<'de> DeserializeAs<'de, primitive_types::H256> for Hex0xH256 {
    fn deserialize_as<D>(deserializer: D) -> Result<primitive_types::H256, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
