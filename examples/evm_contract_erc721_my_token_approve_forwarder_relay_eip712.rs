...
/// cargo run --example evm_contract_erc721_my_token_approve_forwarder_relay_eip712 --features="jsonrpc_client evm" -- [RELAY SERVER HTTP RPC ENDPOINT] [EVM HTTP RPC ENDPOINT] [FORWARDER CONTRACT ADDRESS] [RECIPIENT CONTRACT ADDRESS] [ORIGINAL SIGNER PRIVATE KEY] [TO ADDRESS] [TOKEN ID]
/// cargo run --example evm_contract_erc721_my_token_approve_forwarder_relay_eip712 --features="jsonrpc_client evm" -- http://127.0.0.1:9876/rpc http://127.0.0.1:9650/ext/bc/C/rpc 0x52C84043CD9c865236f11d9Fc9F56aa003c1f922 0x5DB9A7629912EBF95876228C24A848de0bfB43A9 95694eac320cefe9dd7eb4414be4fb67d7ab91aaf0a6a8a79ed95a72ab959bb0 0xf4A19a3C3756B45b93Ae65DAf3F890aC8c054Af5 12345
#[tokio::main]
async fn main() -> io::Result<()> {
    ...
    let to_addr = args().nth(6).expect("no to address given");
    ...
    // parsed function of "approve(address to, uint256 tokenId)"
    ...
    let arg_tokens = vec![Token::Address(to_addr), Token::Uint(token_id)];
    ...
    let relay_tx = Tx::new()
        ...
        .value(U256::zero())
        //
        // assume this is the second transaction after mint
        .nonce(U256::from(1))
        //
